    ...
    public abstract class SingleCodeTable<T> : CodeTable<T> where T : CodeDataModel
    {
        ...
        public IUnitOfData UnitOfData
        {
            get
            {
                if (!DataBase.IsEmpty())
                {
                    string conn = DBString.GetAtawDatabaseValue(DataBase);
                    if (!conn.IsEmpty())
                    {
                        return new AtawDbContext(conn);
                    }
                    else
                ...
                else
                {
                    ...
                    if (!Product.IsEmpty())
                    {
                        return AtawAppContext.Current.FetchUnitofData(app.FControlUnitID, Product);
                    }
                    else
                    {
                        return app.CreateDbContext();
                    }
                }
        ...
        {
            fWhere = string.Format(ObjectUtil.SysCulture, "(  {0} LIKE '{1}%' OR {2} LIKE '%{1}' )", TextField, key, ValueField);
        ...
        //{
        //    fWhere = string.Format(ObjectUtil.SysCulture, "(  {0} LIKE '{1}%' OR {0} LIKE '%{1}%' )", TextField, key, ValueField);
        //    return FillData(postDataSet);
        //    // Where = 
        //}
        ...
        {
            fWhere = string.Format(ObjectUtil.SysCulture, "  {0} LIKE '%{1}%'", TextField, key);
            return FillData(postDataSet);
        }
        ...
        private IEnumerable<T> FillDataTop(string top)
        {
            ...
            select = string.Format(ObjectUtil.SysCulture, select, ValueField, TextField, otherFields);

            string where = "  1 = 1 ";
            if (!string.IsNullOrEmpty(Where))
            {
                where = string.Format(ObjectUtil.SysCulture, " {0} AND {1}", where, Where);
            }

            if (!string.IsNullOrEmpty(fWhere))
            {
                where = string.Format(ObjectUtil.SysCulture, "{0} AND {1}", where, fWhere);
                // where += " AND " +  fWhere;
            }
            //else
            //    where = " 1=1 ";
            ...
            {
                where = "AND ( {0} )".AkFormat(where);
            }
            sql = sql.AkFormat(select, orderName, TableName, where);

           // sql = string.Format(ObjectUtil.SysCulture, sql, select, TableName, where);

            var dbContent = UnitOfData as AtawDbContext;
            //using (dbContent)
            //{
            // string sql = "SELECT ";
            ...
            if (PostDataSet != null)
            {
                ...
                string countSql = string.Format(ObjectUtil.SysCulture, "SELECT COUNT(*) FROM {0} WHERE  (1=1) {1}", TableName, where);
                int totalCount = dbContent.QueryObject(countSql, paraList.ToArray()).Value<int>();
                if (dt != null && dt.Rows.Count > 0)
                {
                ...
                foreach (var qsql in paraList)//对象克隆，防止参数重复
                {
                    SqlParameter sqlP = new SqlParameter();
                    qsql.ObjectClone(sqlP);
                    paraList2.Add(sqlP);
                }
                ds = dbContent.QueryDataSet(sql, paraList2.ToArray());
            }
            else
            {
        ...
        private List<CodeDataModel> getListByDs(DataSet ds)
        {
            ...
            foreach (DataRow row in dt.Rows)
            {
                ...
                model.CODE_TEXT = row["CODE_TEXT"].ToString();
        ...
        public override T this[string key]
        {
            get
            {
                ...
                // get
           // {
                string _classname = this.RegName.IsAkEmpty() ?( this.TableName + this.GetType().Name ): this.RegName;
                var _pf = AtawAppContext.Current.PageFlyweight;

                return _pf.GetAndSet<T>(_classname+ key , () =>

                   {
                       ...
                       {
                           string _key = Sign.IsEmpty() ? RegName : Sign;
                           return AtawAppContext.Current.AtawCache.GetAndSet(
                               string.Format(ObjectUtil.SysCulture, DbCodeTable.CODE_TABLE_KEY_STR, _key, key),
                               () => { return Get(key); }
                               );
                       }
                   }
        ...
        {
            fWhere = " {0} = '{1}' ".AkFormat(ValueField, key);

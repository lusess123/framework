...
namespace Ataw.Framework.Core
{
    ...
    /// 需要解码的控件从此继承
    /// </summary>
    public class AtawDecodeOptionCreator : AtawBaseOptionCreator
    {

        public AtawDecodeOptionCreator()
        {
        }
        ...
        private int[] Decode(string val, string codeTableName, out string text)
        {
            ...
            if (!val.IsEmpty())
            {
                ...
                var codeTable = codeTableName.CodePlugIn<CodeTable<CodeDataModel>>();
                string arrStr = "";
                string dataText = "";
                for (int i = 0; i < result.Length; i++)
                {
                    ...
                    {
                        dataText = codeTable[arrStr].CODE_TEXT;
                        text = text + dataText + ",";
                    }
                    else
                    {
